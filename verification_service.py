...
_SYSTEM_MSG = (
    ...
    "  • wrong — use ONLY when there is POSITIVE evidence of a CONFLICT: the candidate "
    "belongs to a clearly DIFFERENT person/entity (e.g. a same-name namesake whose "
    "content is in a different field — an athlete/sports account for an actor, a "
    "different company for a brand), is a post/video rather than a profile, or is "
    "broken/redirected. A content conflict makes it wrong EVEN IF the handle matches a "
    "declared handle. CRUCIAL: absence of confirming evidence is NOT a conflict — if "
    "the name/handle matches and NOTHING contradicts it, but the content is simply too "
    "thin, generic, private, or empty to confirm the identity, that is manual_review, "
    "NEVER wrong. Do not mark a name-matching profile wrong just because you could not "
    "find proof it IS the person.\n"
    "  • manual_review — use for GENUINE uncertainty: the name/handle matches and "
    "nothing contradicts it, but the content is too thin/generic/private/empty to "
    "confirm the identity; OR several different people plausibly fit and you cannot "
    "distinguish which; OR there is partial conflicting evidence. Do NOT use "
    "manual_review for a plausible, uncontradicted match a reasonable person would "
    "accept (that is verified). A name-matching profile you cannot confirm belongs here "
    "— not in wrong.\n\n"
